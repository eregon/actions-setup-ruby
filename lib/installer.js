...
function downloadTruffleRuby(version) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        // TruffleRuby currently needs the opt and llvm-link tools from LLVM
        // https://github.com/oracle/truffleruby/blob/master/doc/user/installing-llvm.md
        if (platform == 'linux') {
            yield exec.exec('sudo', ['apt-get', 'install', '-y', 'clang', 'llvm']);
        }
        else if (platform == 'darwin') {
            yield exec.exec('brew', ['install', 'llvm@4']);
            let sdkroot = '';
            yield exec.exec('xcrun', ['--sdk', 'macosx', '--show-sdk-path'], { listeners: { stdline: (line) => { sdkroot = line.trimRight(); } } });
